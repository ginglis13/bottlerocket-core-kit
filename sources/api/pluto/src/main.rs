...
/// The path to the AWS config file. The contents of settings.aws.config are decoded and written here.
const AWS_CONFIG_FILE_PATH: &str = "/root/.aws/config.pluto";
...
fn set_aws_config(aws_k8s_info: &SettingsViewDelta, filepath: &Path) -> Result<()> {
    ...
    if let Some(config_contents) = settings_view_get!(aws_k8s_info.aws.config) {
        ...
        let mut file = File::create(filepath).context(error::CreateFileSnafu {
        ...
        log::info!("Wrote AWS config to {:?}", filepath);
...
async fn run() -> Result<()> {
    ...
    // Log the AWS_CONIFG_FILE_ENV_VAR value
    log::info!(
        "pre set config AWS_CONFIG_FILE_ENV_VAR: {}",
        env::var(AWS_CONFIG_FILE_ENV_VAR).unwrap_or_else(|_| "unset".to_string())
    );
    ...
    // Log the AWS_CONIFG_FILE_ENV_VAR value
    log::info!(
        "End of story. AWS_CONFIG_FILE_ENV_VAR: {}",
        env::var(AWS_CONFIG_FILE_ENV_VAR).unwrap_or_else(|_| "unset".to_string())
    );

    // unset the AWS_CONFIG_FILE_ENV_VAR environment variable which was added via `set_aws_config`.
    env::remove_var(AWS_CONFIG_FILE_ENV_VAR);

    log::info!(
        "And finally unset. AWS_CONFIG_FILE_ENV_VAR: {}",
        env::var(AWS_CONFIG_FILE_ENV_VAR).unwrap_or_else(|_| "unset".to_string())
    );
